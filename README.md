...
本プロジェクトでは、M5Stack Tab5の外部出力(GPIO 0)を利用し、WS2812B(NeoPixel)を8個搭載したLEDストリップによるゲーム連動の発光演出システムをFastLEDライブラリを使用し追加実装した。
LEDストリップの制御ピンをM5Stack Tab5のG1ピンに接続し、各ゲームの状況に応じて動的にLEDが発光する。


『主な演出内容』

- 『メインメニュー待機時』: 500ms間隔で1つおきにシアン色（水色）が点滅し、サイバーな待機状態を演出する。
- 『INVADER プレイ中』:
  - 弾発射時: 一瞬(50ms)全体が白く発光する。
  - 敵撃墜時: 全体が緑色でチカチカと点滅(300ms)する。
  - 自機被弾時: 全体が赤色で激しく点滅(600ms)する。
- 『SPIKE プレイ中』:
